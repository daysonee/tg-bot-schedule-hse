    ...
    for column in range(0, len(lecture_columns)):
        ...
        for line in range(12,49, 9):
            if line != 39:
                ...
                ranges = [f"{lecture_columns[column] + str(line)}:{lecture_columns[column] + str(line + 7)}",
                          f"{auditory_columns[column] + str(line)}:{auditory_columns[column] + str(line + 7)}",
                          f"{building_columns[column] + str(line)}:{building_columns[column] + str(line + 7)}",
                          f"B{line}:B{line + 7}", f"C{line}:C{line + 7}"]
                data = (sheet.batch_get(ranges))
            ...
            #может не быть пары на верхней или нижней неделе, надо не забыть проверить длину строки
                        ...
                        if len(auditories) < i+1:
                            upper_week_auditorie, lower_week_auditorie = "-", "-"
                        else:
                ...
                for lesson in schedule_for_day:
                    lesson = [get_lesson(lesson), get_teacher(lesson), lesson[3], lesson[1], lesson[2], get_days(lesson), get_lection_or_seminar(lesson), cnt]
